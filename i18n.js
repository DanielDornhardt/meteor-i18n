...
I18n = {
  ...
  registerLanguage: function(lang, mapName, timeMapName) {
    var map = (Meteor.isServer) ? global[mapName] : window[mapName];
    if (! map)
      throw new Meteor.Error('error_object_not_found');
    this.langPacks[lang] = map;  

    
    if (timeMapName) {
      var timeMap = (Meteor.isServer) ? global[timeMapName] : window[timeMapName];
      if (! timeMap)
        throw new Meteor.Error('error_object_not_found');
        
      if (lang != 'en')
        moment.locale(lang, timeMap);
    }
  ...
  addPartial: function(lang, key, mapName) {
    var map = (Meteor.isServer) ? global[mapName] : window[mapName];
    if (! map)
      throw new Meteor.Error('error_object_not_found');
  ...
  get: function(key, args, lang) {
    var value = "";
    if (typeof args === 'string' && lang === undefined) {
      lang = args;
    } else {
    ...
    try {
      value = key.split(".").reduce(function(o, i) { return o[i] }, this.langPacks[lang]);
